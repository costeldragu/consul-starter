...
#Create an html page with the ip of thin node
ip=$(ifconfig eth1 | grep 'inet addr' | awk '{ print substr($2,6) }')
echo "<h1>$ip $(hotname)</h1>" > /home/vagrant/ip.html

#Run nginx via docker
#mount the ip.html pace as a volume in the root of the default nginx site
